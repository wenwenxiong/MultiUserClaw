...
-  1. 环境配置 (.env.example)
  ...
  2. 删除无用技能 (delete_openclaw_skills.py)
   - 扩展了待删除的技能列表，新增第二批清理：
     - GFW 封锁的服务：gemini、gog、discord
     - 不太有用的：gifgrep、weather、songsee、trello、wacli、sag
     - 桌面/移动端依赖：1password、canvas、node-connect、obsidian
     - 仅开发工具：tmux、himalaya、summarize、video-frames

  3. Agent 记忆系统优化 (deploy_copy/Agents/*/AGENTS.md)
  为所有 Agent（doctor、hr、main、manager、programmer、researcher）统一更新记忆系统：
   - 统一记忆路径：/root/.openclaw/memory/，所有 Agent 共享
   - 引入 qmd 检索系统：
     - 混合搜索：qmd-runner.sh query "<问题>"
     - 精确获取：qmd-runner.sh get <file>:<line> -l 20
   - 即时记忆写入：重要决策完成后立即追加到 YYYY-MM-DD.md，而不是等待 Cron
  ...
  5. Docker 部署优化 (deploy_docker.py)
   - 注释掉数据库容器记录清理逻辑，不再每次部署时删除 containers 表的记录

  6. 前端 - 技能商店重构 (frontend/src/lib/api.ts + SkillStore.tsx)
   - 新增推荐技能功能：
     - 按兴趣分类展示技能（类似苹果应用商店）
     - 水平滚动的分类标签页
     - 每个分类显示图标、名称和技能数量
     - 一键安装推荐技能
   - 新增 API 接口：
     - getRecommendedSkills() - 获取推荐技能分类
     - installRecommendedSkill() - 安装推荐技能
   - UI 改进：
     - 卡片式展示技能
     - 实时显示安装状态（安装中、已安装）
     - 横向滚动支持（带左右箭头）

  7. 前端 - AI模型配置优化 (frontend/src/pages/AIModels.tsx)
   - 支持多次添加同一个模型提供商
   - 自动生成唯一名称（如 anthropic-2、anthropic-3）
   - 改进 UI：移除"已添加"禁用状态，改为"已添加（可再次添加）"提示

  8. 技能市场配置 (marketplaces.json)
   - 将 demo_marketplace 源从 `mrlm-xyz/demo-claude-marketplace` 改为 `johnson7788/collect_skills`

  9. OpenClaw Bridge 增强 (openclaw/)
  Dockerfile.bridge
   - 新增 @tobilu/qmd 全局安装，提供记忆检索能力

  bridge-entrypoint.sh
   - SSH 密钥处理优化：自动移除 Windows 换行符（\r）
   - 全局记忆系统初始化：
     - 创建 memory/、memory/weekly/、memory/archive/ 目录
     - 同步 qmd-runner.sh 脚本
     - 创建 MEMORY.md 模板
     - 初始化 qmd 集合和嵌入
   - 注册定时任务（Cron）：
     - memory-sync：每日 10:00/14:00/18:00/22:00 同步会话到记忆
     - memory-tidy：每日凌晨 3:00 压缩归档记忆文件
   - Agent 工作空间区分：
     - main Agent 使用 workspace/
     - 其他 Agent 使用 workspace-<id>/
     - 平台管理文件（SOUL.md、AGENTS.md、IDENTITY.md）总是覆盖更新
     - 用户文件（USER.md、memory/）只在不存在时创建

  bridge/config.ts
   - 新增 skillsMarketplaceRepo 配置读取

  10. 平台配置 (platform/app/config.py)
   - 新增 public_base_url 配置（默认 http://www.exmaple.com）
   - 用于生成外部访问 URL

  11. 容器管理 (platform/app/container/manager.py)
   - 新增外部访问 URL 生成：
     - 在 Expose Port Skill Markdown 中添加 External Access URLs 节
     - 生成 Service URL 和 Browser URL
     - 明确提示用户不要使用 0.0.0.0 或 localhost，而是使用提供的外部 URL

  12. 新增文件 (deploy_copy/qmd-runner.sh)
   - qmd 包装脚本，确保使用 /root/.openclaw 作为 HOME 目录，保证跨重启持久化
